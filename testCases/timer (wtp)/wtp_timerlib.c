...
#include <stdlib.h>
#include <string.h>
/* 必須在 #include <pthread.h> 之前，先定義巨集 #define _GNU_SOURCE，因為 pthread_getattr_np 屬於 GNU 擴充接口，而不是 POSIX 標準 */
#define _GNU_SOURCE
#include <pthread.h>
...
static void *cronometer(void *arg) {
	...
	/* fprintf(stdout,"\tTicker thread: inizializzaione compleata\n"); */
...
int timer_init() {
	...
	rv = pthread_cond_init(&timerq.cond, &cattr);
	if (rv != 0) {
...
void timer_print() {
	...
	for(t = timerq.first, i = 0; t != NULL; t = t->next, i++) {
		...
		fprintf(stdout,"Timer %d: id=<%d>, expire=<%u,%u>, in_use=<%d>, cancelled=<%d>\n",
